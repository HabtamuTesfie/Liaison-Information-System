...
try {
  ...
  $result = $conn->query('SELECT way, concat((count( * ) *100 / (SELECT count( * ) FROM `referralout`)) , "%") AS percent FROM `referralout` GROUP BY way ');
  $rows = array();
  $table = array();
  $table['cols'] = array(
