...
<a href="viewSentFeedback.php" class="float-left btn btn-dark"><i class="fa fa-arrow-left" style='font-size:24px'></i></a>
...
if(isset($_REQUEST['editId']) and $_REQUEST['editId']!=""){
	$row	=	$db->getAllRecords('feedback','*',' AND fed_id="'.$_REQUEST['editId'].'"');
}
...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	$update	=	$db->update('feedback',$data,array('fed_id'=>$editId));
	if($update){
		header('location: viewSentFeedback.php?msg=rus');
		exit;
	}else{
...
?>
		...
		<form method="post">
              ...
              <fieldset>
						...
						<legend align="center" ><h2>View Sent feedback</h2></legend>
						     <table border = 0 cellspacing= 30 cellpadding=0 align="center">
                      <tr>
                        <td>Diagnosis</td>
					          <td><textarea id="diagnosis" name="diagnosis"  rows="4" cols="40"><?php echo $row[0]['diagnosis'];?></textarea>
				          </td>
                     </tr>
					 <tr>
                        <td>Summary of treatment & Investigation provided:</td>
                        <td> <textarea name="treatment"  rows="4" cols="40"> <?php echo $row[0]['treatment'];?></textarea> </td>
					  </tr>
                     
					 <tr>
                        <td>Outcome</td>
					          <td><textarea id="outcome" name="outcome"  rows="4" cols="40"><?php echo $row[0]['outcome'];?></textarea>
				          </td>
                     </tr>
                        <tr>
                        <td>Feedback sent date:</td>
                        <td> <input type="date" name="creationdate" value="<?php echo $row[0]['creationdate'];?>"  class="text-long" required> </td>
					  </tr>  
                       <tr>
